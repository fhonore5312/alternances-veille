...
load_dotenv()  # OBLIGATOIRE avant os.getenv()
...
def fetch_lba_offers(ville_name, config, romes_codes):
    ...
    headers = {
        "Authorization": f"Bearer {os.getenv('LBA_API_KEY', '')}",
    }
...
SCHOOL_EXCLUSIONS = [
    "enseigne inconnue", "école", "ecole", "formation",
...
def scrape_track(track_key, track_cfg):
    # Scrape toutes les villes pour un track. Retourne la liste d'offres formatees.
    print(f"\n{'─'*60}")
...
def build_output(all_offers, track_keys):
    # Construit le JSON final a partir de toutes les offres (multi-tracks).
