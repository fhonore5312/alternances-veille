...
# Tracks pour lesquels la recherche RH est activée
TRACKS_WITH_HR_CONTACTS = {"digital_marketing", "finance"}


# ===== KNOWN CAREERS (0 appel agent — URL directe hardcodée) =====
KNOWN_CAREERS = {
...
def sanitize_contact(contact: dict) -> dict:
    """Blackliste les emails non-RH — les déplace dans la note."""
...
def clean_json_output(raw: str) -> dict | None:
    """Parse le JSON retourné par result.raw (robuste aux délimiteurs markdown)."""
    raw = raw.strip()
    if raw.startswith("{"):
        try:
...
def build_search_queries(company: str, titre: str) -> tuple[str, str]:
    """Construit les 2 requêtes Serper à passer en inputs au Crew."""
...
def load_offers(track: str | None = "digital_marketing") -> list:
    ...
    if track:
        ...
        offers = [o for o in offers if o.get("track") in TRACKS_WITH_HR_CONTACTS]
    return offers
...
def update_history(history: dict, contact: dict) -> bool:
    """Met à jour l'historique si le nouveau contact a une meilleure confiance."""
...
def run_hr_crew(
    ...
    Conforme CrewAI V2 : résultat via result.raw, pas de fichier temporaire.
    """
...
def run_hr_contacts_agent(
    ...
    """
    Point d'entrée conforme architecture V2.
    Appelé par flow.py → find_hr_contacts().
    Retourne le nombre de contacts actionnables trouvés.
    """
    print("=" * 65)
    print(f"Agent RH CONTACTS v2 — Track : {track or 'ALL'}")
    ...
    # Flush sélectif : purge uniquement les offres du track courant
    if flush and track:
        ...
        print(f"Flush '{track}' : {len(all_existing) - len(existing)} purges, "
    ...
    # Pré-charger le cache entreprises depuis l'historique
...
if __name__ == "__main__":
    ...
    grp.add_argument(
        ...
        help="Track cible (defaut: digital_marketing)",
    )
    grp.add_argument(
    ...
    parser.add_argument(
        ...
        help="Re-recherche le track courant (autres tracks conserves)",
    )
    parser.add_argument(
    ...
    run_hr_contacts_agent(
        track=track, flush=args.flush, flush_history=args.flush_history
    )
